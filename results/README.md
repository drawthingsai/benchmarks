...
Generate a comparison from completed runs with `gguf-bench compare`. Before publication, verify that every GGUF row includes its canonical source URL, revision, exact filename, byte size, SHA-256, and quantization method.
